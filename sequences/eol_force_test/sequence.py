...
class EOLForceTestSequence(SequenceBase):
    ...
    async def _save_test_results(self) -> None:
        ...
        try:
            ...
            # Get WIP ID from context (passed from popup input)
            wip_id = self.context.wip_id
            ...
            if repeat_count > 1 and self._cycle_results:
                await self._save_cycle_measurements(test_id, repeat_count, wip_id)
    ...
    async def _save_cycle_measurements(self, test_id: str, total_cycles: int, wip_id: Optional[str] = None) -> None:
